...
def read_pandas_hdf5(hdf5_file, key=None, ret=False):
	...
	hdf5_file_keys = hdf5_file_obj.keys()
	hdf5_file_obj.close()
	if key==None:
		try:
...
def read_hdf5_file_to_pandas_DF(hdf5_file, key=None):
	...
	pd_dict = {}
	if not key:
		data_keys = num.array(data_file.keys())
		if len(data_keys)==1:
			data = data_file.get(data_file.keys()[0])
...
def pandas_file_to_hdf5_file(pandas_hdf5_file, hdf5_file, key=None, mode='a'):
	"""
...
def pandas_df_to_hdf5_file(data, hdf5_file, kind='pandas', key=None, mode='a'):
	"""
...
def concadenate_pd_df(direc, file_type='hdf5', foutput=None, outonly=True):
	...
	if len(df_files) > 0:
		...
		if (foutput is not None) and (type(foutput)==str):
			...
			pandas_df_to_hdf5_file(df_conc, foutput_file, kind='pandas', 
				key='/Main')
