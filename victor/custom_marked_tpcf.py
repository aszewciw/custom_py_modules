...
"""
Computes the two-point marked correlation function, but it was customized to 
return number of weighted pairs and normal pairs separately.
"""

from __future__ import division, print_function
__author__     =['Victor Calderon']
__copyright__  =["Copyright 2016 Victor Calderon, custom_marked_tpcf"]
__email__      =['victor.calderon@vanderbilt.edu']
__maintainer__ =['Victor Calderon']
__all__        =["custom_marked_tpcf"]

import numpy as num
import sys
from   math import pi, gamma
from   halotools.mock_observables.clustering_helpers import *
from   halotools.mock_observables.pair_counters.marked_double_tree_pairs import marked_npairs
from   halotools.mock_observables.pair_counters.double_tree_pairs import npairs

num.seterr(divide='ignore', invalid='ignore') #ignore divide by zero in e.g. DD/RR

def custom_marked_tpcf(sample1, rbins, sample2=None, 
    ...
    return_counts: boolean, optional
        If True, it will calculate the actual number of pairs.
    ...
    if return_counts==True:
        ...
        #calculate randomized marked pairs
            ...
            if iterations > 1:
                ...
                R1R1 = num.zeros((iterations,len(rbins)-1))
                R1R2 = num.zeros((iterations,len(rbins)-1))
                R2R2 = num.zeros((iterations,len(rbins)-1))
                for i in range(iterations):
                    print(i)
                    #get arrays to randomize marks
                    permutate1 = num.random.permutation(num.arange(0,len(sample1)))
                    permutate2 = num.random.permutation(num.arange(0,len(sample2)))
                    R1R1[i,:],R1R2[i,:],R2R2[i,:] = random_counts(sample1, sample2, \
                                                    rbins, period,\
                                                    num_threads, do_auto, do_cross,\
                                                    marks1, marks2, wfunc,\
                                                    _sample1_is_sample2,\
                                                    permutate1, permutate2, \
                                                    randomize_marks)
                #take mean of the iterations
                R1R1_err = num.std(R1R1, axis=0)
                R1R1 = num.median(R1R1, axis=0)
                R1R2_err = num.std(R1R2, axis=0)
                R1R2 = num.median(R1R2, axis=0)
                R2R2_err = num.std(R2R2, axis=0)
                R2R2 = num.median(R2R2, axis=0)
            else:
                #get arrays to randomize marks
                permutate1 = num.random.permutation(num.arange(0,len(sample1)))
                permutate2 = num.random.permutation(num.arange(0,len(sample2)))
